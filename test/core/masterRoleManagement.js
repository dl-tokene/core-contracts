...
describe("MasterRoleManagement", async () => {
  ...
  before("setup", async () => {
    ...
    await masterRoles.grantRole(CONSTANTS_REGISTRY_ADMIN_ROLE, USER3);
  ...
  describe("getters", async () => {
    ...
    it("should correctly check the role for hasConstantsRegistryAdminRole", async () => {
      assert.isTrue(await masterRoles.hasConstantsRegistryAdminRole(USER3));
      await masterRoles.revokeRole(CONSTANTS_REGISTRY_ADMIN_ROLE, USER3);
      assert.isFalse(await masterRoles.hasConstantsRegistryAdminRole(USER3));
    });
  });
});
